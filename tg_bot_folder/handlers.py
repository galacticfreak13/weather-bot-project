...
async def input_city(message: types.Message, state: FSMContext):
    await state.set_state(FutureDaysStates.waiting_input_city)
    await message.answer('Введи любой город!')

async def send_input_city(message: types.Message, state: FSMContext):
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(WEATHER_KEY2, params=my_params) as response:
            if response.status == 200:
                my_data = await response.json()
                if 'results' not in my_data:
                    await message.answer(f'Город под названием {city_user} не найден.')
                    await state.clear()
                else:
                    first_element = my_data['results'][0]
                    my_params = {'latitude': first_element['latitude'], 'longitude': first_element['longitude'],
                                 'timezone': 'auto',
                                 'hourly': 'temperature_2m,weather_code'}

                    async with aiosqlite.connect(DB_NAME) as db:
                        await db.execute(
                            'INSERT INTO users(user_id, latitude_db, longitude_db, time_db, cities_db) VALUES(?,?,?,?,?)',
                            (id_user, first_element['latitude'], first_element['longitude'], time,
                             first_element['name']))
                        await db.commit()
...
async def reg_on_geo(message: types.Message, state: FSMContext):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if str(days).isdigit():
            ...
            my_params = {'latitude': my_latitude, 'longitude': my_longitude, 'timezone': 'auto', 'hourly': 'temperature_2m,weather_code', 'start_date': days, 'end_date': days}
        ...
        async with session.get(WEATHER_KEY, params=my_params) as response:
            if response.status == 200:
                ...
                weather_code = my_data['hourly']['weather_code'][0]
                text = ''
                count_day = 0
                for codes, states in weather_dictionary.items():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(WEATHER_KEY2, params=my_params) as response:
            if response.status == 200:
                ...
                first_element = my_data['results'][0]
                if str(days).isdigit():
                    my_params2 = {'latitude': first_element['latitude'], 'longitude': first_element['longitude'],'timezone': 'auto','hourly': 'temperature_2m,weather_code', 'forecast_days':days}
                else:
                    my_params2 = {'latitude': first_element['latitude'], 'longitude': first_element['longitude'], 'timezone': 'auto',
                                 'hourly': 'temperature_2m,weather_code', 'start_date': days, 'end_date': days}
...
router.message.register(send_input_city, F.text, FutureDaysStates.waiting_input_city)
